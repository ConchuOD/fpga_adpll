...
\title{FPGA Based Modelling of an ADPLL Network\vspace{-0.3cm}}
\author{
	\IEEEauthorblockN{C. Dooley$^1$, E. Blokhina$^1$, B. Mulkeen$^1$, D. Galayko$^2$}
    \IEEEauthorblockA{\textit{$^1$School of Electrical \& Electronic Engineering, University College Dublin}\\
    \textit{$^2$LIP6 Lab, Sorbonne Universit\'e}
    	\vspace{-0.7cm}
	}
}
\IEEEoverridecommandlockouts
\IEEEpubid{\makebox[\columnwidth]{978-1-7281-1201-5/19/\$31.00~\copyright2019 IEEE \hfill} \hspace{\columnsep}\makebox[\columnwidth]{ }}
\maketitle
...
In combination with the bit width of the control signal, the minimum frequency step limits the range over which the oscillator can be tuned. Frequently the output of the DCO is divided down to allow the control blocks in the feedback path that are clocked on this signal, to run at a lower and thus easier frequency.\vspace{-0.2cm}
...
\begin{table}[!ht]
    \begin{center}
        \begin{tiny}
            ...
            \end{tabular}
        \end{tiny}
        \caption{ADPLL Performance Comparison.}
